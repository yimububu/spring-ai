...
/**
 * AbstractEmbeddingModel 是 {@link EmbeddingModel} 接口的一个抽象实现，提供了嵌入向量维度计算的缓存功能。
 *
...
public abstract class AbstractEmbeddingModel implements EmbeddingModel {
    ...
    /**
     * 返回请求的嵌入模型（embedding generative）名称的维度。如果该名称未知，则使用 EmbeddingModel 执行一个虚拟的 EmbeddingModel#embed 操作，并计算返回的嵌入向量维度。
     *
    ...
    /**
     * 延迟初始化 embeddingDimensions 变量，并确保其只被初始化一次。
     */
    @Override
